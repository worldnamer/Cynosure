 ...
 * <code>new InitialContext()</code> will expose the required objects. Also usable for standalone applications, e.g. for
 ...
 * </pre>
 * 
 * Note that you <i>should not</i> call <code>activate()</code> on a builder from this factory method, as there will
 * already be an activated one in any case.
 * <p>
...
public class SimpleNamingContextBuilder implements InitialContextFactoryBuilder
{
	 ...
	 * 
	 * @return the current SimpleNamingContextBuilder instance, or <code>null</code> if none
	 */
	...
	/**
	 * If no SimpleNamingContextBuilder is already configuring JNDI, create and activate one. Otherwise take the
	 * existing activate SimpleNamingContextBuilder, clear it and return it.
	 * <p>
	 ...
	 * <code>new InitialContext()</code> will always return a context from this factory. Use the
	 * <code>emptyActivatedContextBuilder()</code> static method to get an empty context (for example, in test methods).
	 * 
	 ...
	 * <p>
	 * Call <code>activate()</code> again in order to expose this context builder's own bound objects again. Such
			...
			if (icf != null)
			{
				...
				catch (Throwable ex)
				{
					IllegalStateException ise = new IllegalStateException(
							"Cannot instantiate specified InitialContextFactory: " + icf);
					ise.initCause(ex);
					throw ise;
				}
			}
